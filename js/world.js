...
export class World {
  ...
  setNest(x, y, radius = 14) {
    this.nest = { x, y, radius };
  }

  addFood(x, y, radius = 18, amount = Infinity) {
    this.foodSources.push({ x, y, radius, amount });
  }
  ...
  eraseAt(x, y, radius = 18) {
    if (this.nest) {
      const dx = x - this.nest.x, dy = y - this.nest.y;
      const r = this.nest.radius + radius;
      if (dx * dx + dy * dy <= r * r) this.nest = null;
    }
    this.foodSources = this.foodSources.filter(f => {
      const dx = x - f.x, dy = y - f.y;
      const r = f.radius + radius;
      return dx * dx + dy * dy > r * r;
    });
    this.paintObstacleCircle(x, y, radius, false);
  }
