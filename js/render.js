...
// Renderer owns three offscreen surfaces:
//   - pherCanvas: grid-resolution heatmap, drawn once per frame, scaled up
//   - obstacleCanvas: full-resolution wall layer, redrawn only when dirty
// Plus the main canvas for live entities (ants, food, nest).
...
export class Renderer {
  constructor(canvas, sim) {
    ...
    // Camera transform plumbed through every draw, even though Phase 1 leaves
    // it at identity. Retrofitting a camera into a renderer that bakes screen
    // coords directly is tedious — so we pay it now.
